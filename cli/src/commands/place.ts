...
export async function placeCommand(args: string[], globals: GlobalFlags): Promise<void> {
  ...
  const apiKey = process.env.GOOGLE_PLACES_API_KEY || loadConfig().googlePlacesApiKey;
  if (!apiKey) {
    throw new WaiError(
      'No Google Places API key. Set GOOGLE_PLACES_API_KEY or add googlePlacesApiKey to ~/.config/wai/config.json.',
      1,
    );
  }
  ...
  if (globals.json) {
    ...
    for (const p of results) {
      ...
      if (p.rating) {
        console.log(`    ${p.rating}★${p.userRatingCount ? ` (${p.userRatingCount} reviews)` : ''}`);
      }
      if (p.open !== undefined) {
